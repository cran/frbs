...
control <- list(popu.size = 80, persen_cross = 0.6, max.iter = 300, persen_mutant = 0.3, epsilon = 0.05, name="sim-0")
...
res.fit <- frbs.test(object, data.fit)

##########################
### 3b. Testing process
res.test <- frbs.test(object, data.tst)
...
print("Comparison MSGFS Vs Real Value on Gas Furnace Data Set")
print(bench)
